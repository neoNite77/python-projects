...
class TicTacToe:
    ...
    def print_board_nums():
        # 0 | 1 | 2 etc (tells us what number corressponds to what box)
        number_board = [[str(i) for i in range(j*3, (j+1)*3)] for j in range(3)]
        for row in number_board:
            print("| " + " | ".join(row) + " |")

    def available_moves(self):
        # Method #1: List comphrension
    ...
    def make_move(self, square, letter):
        # if valid move, th en make the move(assign square to letter)
    ...
    def winner(self, square, letter):
        # winnder if 3 in a row anywhere.. we have to check all of these!
        ...
        # if all of the checks fail, return False
        return False
